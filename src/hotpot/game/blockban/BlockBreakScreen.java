...
public class BlockBreakScreen extends Screen {
	...
	public BlockBreakScreen(Game game) {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 3; j++) {
				Block newBlock = new Block(30 + i * 33, 53 + j * 17, 32, 16,
						Color.BLUE);
	...
	public void update(float deltaTime) {
		...
		for (Block block : blocks) {
			...
			if (bollDirection == BollDirection.LEFT_DOWN
				...
				// block.isVisivle = false;
				// } else if (bollDirection == BollDirection.LEFT_UP
				// && (bollTop - syoritien < block.top + block.height && bollTop
				// < block.top)
				// && (bollLeft > block.left && bollLeft < block.left
		...
		// 操作台に触れたとき、玉の方向を変更
		if (bollDirection == BollDirection.LEFT_DOWN
				&& (bollTop + bollCy > daiTop && bollTop < daiTop + daiHeight)
				&& (bollLeft > daiLeft && bollLeft < daiLeft + daiWidth)) {// 左下に移動中に台に触れたら左上に移動
			bollDirection = BollDirection.LEFT_UP;
		} else if (bollDirection == BollDirection.RIGHT_DOWN
				&& bollTop + bollCy > daiTop
				&& (bollLeft > daiLeft && bollLeft < daiLeft + daiWidth)) {// 左下に移動中に台に触れたら左上に移動
			bollDirection = BollDirection.RIGHT_UP;
		}
	...
	public void present(float deltaTime) {
		...
		// g.drawRect(0, 0, 16, 480, Color.BLACK);// 左の壁
		// g.drawRect(320 - 16, 0, 16, 480, Color.BLACK);// 右の壁
		// g.drawRect(0, 0, 320, 16, Color.BLACK);// 上の壁
		...
		for (Block block : blocks) {
			...
			g.drawRect(block.left, block.top, block.width, block.height,
					block.color);
		}
